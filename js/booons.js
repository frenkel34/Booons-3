		...
		console.log('gathering data from json');
			$(function(){
				...
				$.each(data.users, function(index, value){
					 sOutputUsers += '<li class="ui-icon-user ui-btn-icon-left" style="padding-left:35px;">' + value.active + ': ' + value.username + '</li>';
				});
				 sOutputUsers += '';
				...
				$.each(data.events, function(index, value){
					 sOutputEvents += '<li class="ui-icon-' + value.icon + ' ui-btn-icon-left" style="padding-left:35px;">' + value.time + ': ' + value.message + '</li>';
				});
				 sOutputEvents += '';
				 $("#lstEventlist").html(sOutputEvents);
				console.log('refreshing listview for css');
						 	...
						 	if (data.round.runnername == $("#dbgUsername").val()) {
						 		...
						 		if (typeof data.order != 'undefined'){
									...
									$.each(data.order, function(index, value){
										 sOutputOrder += '<li>' + value.product + ' voor ' + value.name + '</li>';
									});
									 sOutputOrder += '';
...
function updateLists() {
	if ( $('#lstUserlist').hasClass('ui-listview')) {
		setTimeout(function() {$('#lstUserlist').listview('refresh'); },100);
		setTimeout(function() {console.log('refreshed userlist'); },100);
     } 
	if ( $('#lstEventlist').hasClass('ui-listview')) {
		setTimeout(function() {$('#lstEventlist').listview('refresh'); },100);
		setTimeout(function() {console.log('refreshed eventlist'); },100);
     } 
	if ( $('#lstRoundlist').hasClass('ui-listview')) {
		setTimeout(function() {$('#lstRoundlist').listview('refresh'); },100);
		setTimeout(function() {console.log('refreshed roundorderlist'); },100);
     } 
